import Foundation
import CoreLocation

// MARK: - Cafe

struct Cafe: Identifiable, Codable, Equatable {
    var id: UUID = UUID()
    var name: String
    var neighborhood: String
    var latitude: Double
    var longitude: Double
    var vibeTags: [String]
    var totalSeats: Int
    var seatsAvailable: Int
    var pricePerSession: Double   // dollars
    var sessionMinutes: Int
    var isParticipating: Bool
    var ownerID: String

    var coordinate: CLLocationCoordinate2D {
        CLLocationCoordinate2D(latitude: latitude, longitude: longitude)
    }

    static func == (lhs: Cafe, rhs: Cafe) -> Bool { lhs.id == rhs.id }
}

// MARK: - Session

struct Session: Identifiable, Codable {
    var id: UUID = UUID()
    var cafeID: UUID
    var cafeName: String
    var startTime: Date
    var endTime: Date
    var price: Double
    var userName: String

    var timeRemaining: TimeInterval { endTime.timeIntervalSinceNow }
    var isActive: Bool { timeRemaining > 0 }
}

// MARK: - Transaction

struct Transaction: Identifiable, Codable {
    var id: UUID = UUID()
    var date: Date
    var description: String
    var amount: Double  // positive = credit, negative = charge
}

// MARK: - Demo Data

extension Cafe {
    static var demoData: [Cafe] {
        [
            Cafe(name: "Sightglass Coffee",
                 neighborhood: "SoMa",
                 latitude: 37.7749, longitude: -122.4194,
                 vibeTags: ["Quiet", "Fast WiFi", "Outlets"],
                 totalSeats: 12, seatsAvailable: 8,
                 pricePerSession: 2.0, sessionMinutes: 90,
                 isParticipating: true, ownerID: "merchant1"),

            Cafe(name: "Ritual Coffee",
                 neighborhood: "Hayes Valley",
                 latitude: 37.7759, longitude: -122.4245,
                 vibeTags: ["Lively", "Good Espresso"],
                 totalSeats: 8, seatsAvailable: 3,
                 pricePerSession: 2.0, sessionMinutes: 90,
                 isParticipating: true, ownerID: "merchant1"),

            Cafe(name: "Four Barrel Coffee",
                 neighborhood: "Mission",
                 latitude: 37.7643, longitude: -122.4215,
                 vibeTags: ["Cozy", "Bookshelf", "Quiet"],
                 totalSeats: 10, seatsAvailable: 10,
                 pricePerSession: 2.0, sessionMinutes: 90,
                 isParticipating: false, ownerID: "merchant2"),

            Cafe(name: "Verve Coffee",
                 neighborhood: "Castro",
                 latitude: 37.7609, longitude: -122.4350,
                 vibeTags: ["Outdoor Seating", "Fast WiFi"],
                 totalSeats: 6, seatsAvailable: 6,
                 pricePerSession: 2.0, sessionMinutes: 90,
                 isParticipating: true, ownerID: "merchant2"),
        ]
    }
}
